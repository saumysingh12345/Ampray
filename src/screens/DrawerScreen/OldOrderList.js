...
export default function OldOrderScreen() {
  ...
  useEffect(() => {
    ...
    firebase.database().ref('Current User Data/' + currentUUIDofUser.toString() + '/' + 'Order Confired Data/').on('value', (snapshot) => {
      var update = []
      var totalIt = 0
      snapshot.forEach((child) => {
        ...
        update.push({
          key: child.key,
          Date: child.val().date,
          TotalPayment: child.val().totalPayyyy,
          imageUrl:child.val().confiredOrderArray['0']['imageUrl']
        })
      })
     // console.log(update)
  ...
  const goToSubCategoryScreen = (message, totalPay) => {

    navigation.navigate('OldOrderDetailsScreen', {
      message,
      totalPay
    });
  };
        ...
        <ScrollView style={{ flex: 1 }}>
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              ...
              <TouchableOpacity
                onPress={() => goToSubCategoryScreen(item.key.toString(),item.TotalPayment)}
              >
